...
enum DAMAGE_QUADRANT // bitfield
{
	...
	// missing left/right back/front here
};
...
public:
	...
	virtual void NotifyGeneratedDeath(DWORD weenie_id);
	virtual void OnGeneratedDeath(DWORD weenie_id);
	virtual void NotifyGeneratedPickedUp(DWORD weenie_id);
	virtual void OnGeneratedPickedUp(DWORD weenie_id);

	virtual void TryIdentify(CWeenieObject *);
	virtual void Identify(CWeenieObject *);
	...
	int GetStructureNum();
	void DecrementStackOrStructureNum(bool bDestroyOnZero = true);
	void DecrementStackNum(bool bDestroyOnZero = true);
	void DecrementStructureNum(bool bDestroyOnZero = true);
	...
	void CreateGeneratorSpawns(bool init);
	void CreateGeneratorSpawn(const GeneratorProfile &profile);
	void CreateGeneratorSpawn(const GeneratorProfile &profile, DWORD wcid, int ptid, float shade);
	...
	float HeadingDiffTo(CWeenieObject *target);
	...
	DWORD GiveSkillXP(STypeSkill key, DWORD amount);
	...
	virtual void ResistedSpell(CWeenieObject *attacker);
	virtual void EvadedAttack(CWeenieObject *attacker);
	...
	bool _useHeartBeat = false;
	double _nextHeartBeat = 0.0;
	...
	virtual double GetDefenseMod();
	virtual double GetDefenseModUsingWielded();
	...
	double GetMeleeCripplingBlowMultiplier(DWORD meleeSkill);
	double GetMissileCripplingBlowMultiplier(DWORD missileSkill);

	double GetCriticalFrequency();
	double GetMeleeCriticalStrikeFrequency(DWORD meleeSkill);
	double GetMissileCriticalStrikeFrequency(DWORD missileSkill);
	double GetCasterCriticalStrikeFrequency(DWORD missileSkill);

	DWORD GetImbuedEffects();
	...
	DWORD _lastOpenedRemoteContainer = 0;
