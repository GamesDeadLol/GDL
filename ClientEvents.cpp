...
void CClientEvents::LoginCharacter(DWORD char_weenie_id, const char *szAccount)
{
	...
	*/
	...
	// m_pPlayer->SendText("PhatAC build: " SERVER_VERSION_NUMBER_STRING " " SERVER_VERSION_STRING, LTT_DEFAULT);
...
void CClientEvents::TryStackableMerge(DWORD merge_from_id, DWORD merge_to_id, DWORD amount)
{
	m_pPlayer->TryStackableMerge(merge_from_id, merge_to_id, amount);
}

void CClientEvents::TryStackableSplitToContainer(DWORD stack_id, DWORD container_id, DWORD place, DWORD amount)
{
	m_pPlayer->TryStackableSplitToContainer(stack_id, container_id, place, amount);
}

void CClientEvents::TryStackableSplitTo3D(DWORD stack_id, DWORD amount)
{
	m_pPlayer->TryStackableSplitTo3D(stack_id, amount);
}

void CClientEvents::TryStackableSplitToWield(DWORD stack_id, DWORD loc, DWORD amount)
{
	m_pPlayer->TryStackableSplitToWield(stack_id, loc, amount);
}

void CClientEvents::TryGiveObject(DWORD target_id, DWORD object_id, DWORD amount)
{
	if (m_pPlayer->IsBusyOrInAction())
	{
		m_pPlayer->NotifyInventoryFailedEvent(object_id, WERROR_ACTIONS_LOCKED);
		return;
	}

	int error = m_pPlayer->TryGiveObject(target_id, object_id, amount);

	if (error != WERROR_NONE)
	{
		m_pPlayer->NotifyInventoryFailedEvent(object_id, error);
	}
}

void CClientEvents::EquipItem(DWORD dwItemID, DWORD dwCoverage)
{
	if (!m_pPlayer->TryEquipItem(dwItemID, dwCoverage))
	{
		 m_pPlayer->NotifyInventoryFailedEvent(dwItemID, 0);
	}
...
bool CClientEvents::StoreItem(DWORD item_id, DWORD container_id, DWORD slot)
{
	if (m_pPlayer->IsBusyOrInAction())
		return false;
	...
	CContainerWeenie *pContainer = m_pPlayer->FindContainer(container_id);
	if (!pContainer)
		return false;

	CWeenieObject *pItem = g_pWorld->FindWithinPVS(m_pPlayer, item_id);
	if (!pItem)
		return false;
	...
	if (!pContainer->Container_CanStore(pItem))
		return false;
	if (pItem->IsCreature()) // Only store/move ordinary items for now.
		return false;

	if (!m_pPlayer->FindContainedItem(pItem->GetID()))
	{
		CPickupInventoryUseEvent *pickupEvent = new CPickupInventoryUseEvent();
		pickupEvent->_target_id = item_id;
		pickupEvent->_target_container_id = container_id;
		pickupEvent->_target_slot = slot;
		m_pPlayer->ExecuteUseEvent(pickupEvent);
	...
	{
		if (pContainer->GetTopLevelID() != m_pPlayer->GetID())
		{
			m_pPlayer->NotifyInventoryFailedEvent(item_id, WERROR_ATTUNED_ITEM);
			return true;
		}
	}

	if (!m_pPlayer->Container_TryStoreItemNow(item_id, container_id, slot))
		return false;

	// m_pPlayer->NotifyUseDone();
	return true;
}

bool CClientEvents::DropItem(DWORD item_id)
{
	// Scenarios to consider:
	// 1. Item being stored is equipped.
	...
	// Find the item.
	CWeenieObject *pItem = m_pPlayer->FindContainedItem(item_id);
	if (!pItem)
		return false;
	...
	dropEvent->_target_id = item_id;
	m_pPlayer->ExecuteUseEvent(dropEvent);
...
void CClientEvents::BuyHouse(DWORD slumlord, const PackableList<DWORD> &items)
{
...
void CClientEvents::RentHouse(DWORD slumlord, const PackableList<DWORD> &items)
{
	if (CWeenieObject *slumlordObj = g_pWorld->FindObject(slumlord))
	{
		if (m_pPlayer->DistanceTo(slumlordObj, true) > 10.0)
	...
	switch (dwEvent)
	{
		...
		case 0x0019: //Store Item
			{
				...
				if (!StoreItem(dwItemID, dwContainer, (char)dwSlot))
				{
					m_pPlayer->NotifyInventoryFailedEvent(dwItemID, 0);
				}
		...
		case 0x0054: // Evt_Inventory__StackableMerge
			{
				...
				TryStackableMerge(merge_from_id, merge_to_id, amount);
				break;
			}
		case 0x0055: // Evt_Inventory__StackableSplitToContainer
			{
				...
				TryStackableSplitToContainer(stack_id, container_id, place, amount);
				break;
			}
		case 0x0056: // Evt_Inventory__StackableSplitTo3D
			{
				...
				TryStackableSplitTo3D(stack_id, amount);
				break;
			}
		case 0x019B: // Evt_Inventory__StackableSplitToWield
			{				
				...
				TryStackableSplitToWield(stack_id, loc, amount);
				break;
			}
		...
		case 0x00CD: // Put object in container
			{
				...
				TryGiveObject(target_id, object_id, amount);
				break;
			}		
		...
		case 0x021C: // Buy House
			{
				...
				BuyHouse(slumlord, items);
				break;
			}
		case 0x0221: // Rent House
			{
				DWORD slumlord = pReader->Read<DWORD>();
