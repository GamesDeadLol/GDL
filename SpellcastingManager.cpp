...
Position CSpellcastingManager::GetSpellProjectileSpawnPosition(CSpellProjectile *pProjectile, CWeenieObject *pTarget, float *pDistToTarget)
{
	...
	Vector targetOffset = spawnPosition.get_offset(pTarget->m_Position.add_offset(Vector(0, 0, pTarget->GetHeight() * (bArc ? (5.0 / 6.0) : (0.5)))));
...
{
	Vector targetOffset = pSpawnPosition->get_offset(pTarget->m_Position.add_offset(Vector(0, 0, pTarget->GetHeight() * 0.5f)));
		...
		for (int y = 0; y < numY; y++)
		{
			for (int z = 0; z < numZ; z++)
			{
				...
				CSpellProjectile *pProjectile = new CSpellProjectile( // revise me
					m_SpellCastData, meta->_numProjectiles > 1 ? 0 : m_SpellCastData.target_id, damage);
...
int CSpellcastingManager::LaunchSpellEffect()
{
	...
	if (m_pWeenie->AsPlayer() && m_SpellCastData.uses_mana)
	{
		...
		if (m_pWeenie->m_Position.distance(m_SpellCastData.initial_cast_position) >= 6.0)
		{
			// fizzle
			m_pWeenie->EmitEffect(PS_Fizzle, 0.542734265f);
			m_pWeenie->AdjustMana(-5);
		...
		case SpellType::Boost_SpellType:
			{
				...
				int boostMin = meta->_boost;
				int boostMax = meta->_boost + meta->_boostVariance;
				if (boostMin > boostMax)
				{
					int swapper = boostMin;
					boostMin = boostMax;
					boostMax = swapper;
				}
						...
						if (m_pWeenie != target)
						{
							...
							if (m_SpellCastData.spell->_bitfield & Resistable_SpellIndex)
							{
								if (topLevelOwner->TryMagicResist(m_SpellCastData.current_skill))
								{
									...
									topLevelOwner->ResistedSpell(m_pWeenie);
									topLevelOwner->ChanceExecuteEmoteSet(m_pWeenie->GetID(), ResistSpell_EmoteCategory);
									continue;
								}
							}
							...
							if (int resistMagic = target->InqIntQuality(RESIST_MAGIC_INT, 0, FALSE))
							{
								if (resistMagic >= 9999 || ::TryMagicResist(m_SpellCastData.current_skill, (DWORD) resistMagic))
								{
									...
									m_pWeenie->SendText(csprintf("%s resists your spell", target->GetName().c_str()), LTT_MAGIC);
									target->ResistedSpell(m_pWeenie);
									target->ChanceExecuteEmoteSet(m_pWeenie->GetID(), ResistSpell_EmoteCategory);
									continue;
								}
							}
						}
	...
	if (m_pWeenie != source)
	{
		...
		if (m_SpellCastData.spell->_bitfield & Resistable_SpellIndex)
		{
			if (source->TryMagicResist(m_SpellCastData.current_skill))
			{
				...
				source->ResistedSpell(m_pWeenie);
				source->ChanceExecuteEmoteSet(m_pWeenie->GetID(), ResistSpell_EmoteCategory);
				return false;
			}
		}
...
bool CSpellcastingManager::AdjustVital(CWeenieObject *target, int amount, DAMAGE_TYPE dt)
{
	if (!target || target->IsDead())
		return false;
	...
	if (amount < 0)
	{
		if (target->ImmuneToDamage(m_pWeenie))
		{
			return false;
		}
	...
	case HEALTH_DAMAGE_TYPE:
		{
			amount = target->AdjustHealth(amount);
			vitalName = "health";
			break;
		}
	case STAMINA_DAMAGE_TYPE:
		{
			amount = target->AdjustStamina(amount);
			vitalName = "stamina";
			break;
		}
	case MANA_DAMAGE_TYPE:
		{
			amount = target->AdjustMana(amount);
			vitalName = "mana";
			break;
		}
	default:
	...
	SendAdjustVitalText(target, amount, vitalName);

	target->CheckDeath(GetCastSource(), dt);
