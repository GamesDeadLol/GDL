...
CLIENT_COMMAND(debug, "<index>", "", ADMIN_ACCESS)
{
	...
	{
		info += csprintf(" Reset: %.1fm", resetInterval / 60.0);
	}
...
CLIENT_COMMAND(spawnperks, "", "Spawns a aura caster for contributors.", DONOR_ACCESS)
{
	...
	CWeenieObject *weenie = g_pWeenieFactory->CreateWeenieByClassID(100004, 0, false);

	weenie->SetID(g_pWorld->GenerateGUID(eDynamicGUID));

	if (pPlayer->Container_TryStoreItemNow(weenie, pPlayer->GetID(), 0, true))
	{
		if (!g_pWorld->CreateEntity(weenie))
			return false;

		pPlayer->MakeAware(weenie);
	}
...
CLIENT_COMMAND(instakill, "<radius>", "Deals damage to all nearby creatures.", ADMIN_ACCESS)
{
	...
	for (auto &entry : results)
	{
		...
		dmgEvent.outputDamage = dmgEvent.inputDamage = 1000000;
...
CLIENT_COMMAND(spawntreasure, "<tier>", "Spawn treasure of a specific tier", SENTINEL_ACCESS)
{
	if (argc < 1)
		return true;

	if (CLootTierFactory *tier = g_pTreasureFactory->GetLootTier(atoi(argv[0])))
	{
		if (CWeenieObject *weenie = tier->CreateRandomLoot())
		{
			weenie->SetInitialPosition(pPlayer->GetPosition());
...
CLIENT_COMMAND(spawntreasure2, "<wcid> <tier> <num>", "Spawn treasure of a specific tier", ADMIN_ACCESS)
{
	if (argc < 3)
		return true;
	...
	for (int i = 0; i < num; i++)
	{
		...
		int tmd_ = 0;
		if (weenie->m_Qualities.InqInt(TSYS_MUTATION_DATA_INT, tmd_, TRUE, TRUE) && tmd_)
		{
			DWORD tmd = (DWORD) tmd_;

			BYTE spell = (tmd >> 24) & 0xFF;
			BYTE color = (tmd >> 16) & 0xFF;
			BYTE gem = (tmd >> 8) & 0xFF;
			BYTE material = (tmd >> 0) & 0xFF;

			MaterialType baseMat = g_pPortalDataEx->_treasureTableData.RollBaseMaterialFromMaterialCode(material, tier);
			MaterialType mat = g_pPortalDataEx->_treasureTableData.RollMaterialFromBaseMaterial(baseMat, tier);
			int ptid = g_pPortalDataEx->_treasureTableData.RollPaletteTemplateIDFromMaterialAndColorCode(mat, color, tier);

			weenie->m_Qualities.SetInt(MATERIAL_TYPE_INT, mat);

			weenie->m_Qualities.SetInt(VALUE_INT, (int)(weenie->InqIntQuality(VALUE_INT, 0, TRUE) * (1.0f + g_pPortalDataEx->_treasureTableData.RollValueEnchantmentForMaterial(mat, tier))));

			std::string materialName;
			if (MaterialTypeEnumMapper::MaterialTypeToString(mat, materialName))
			{
...
CLIENT_COMMAND(spawntreasure3, "<tier> <num>", "Spawn treasure of a specific tier", ADMIN_ACCESS)
{
	if (argc < 2)
		return true;

	int tier = max(1, min(6, atoi(argv[0])));
	int num = min(100, atoi(argv[1]));

	for (int i = 0; i < num; i++)
	{
		CWeenieObject *weenie = g_pTreasureFactory->RollLootForTier(tier);
		...
		weenie->SetInitialPosition(pPlayer->m_Position.add_offset(Vector(Random::GenFloat(-3.0, 3.0), Random::GenFloat(-3.0, 3.0), 1.0)));
...
/*
CLIENT_COMMAND(spawnmutate, "<name>", "Spawn by wcid and mutate.", ADVOCATE_ACCESS)
{
	if (g_pConfig->GetValue("weapons_testing", "0") == 0)
	{
	...
	if (CMutationFilter *filter = g_pPortalDataEx->GetMutationFilter(weenie->InqDIDQuality(TSYS_MUTATION_FILTER_DID, 0)))
	{
		pPlayer->SendText("Mutating...", LTT_DEFAULT);

		filter->TryMutate(&weenie->m_Qualities);
	}
	else
	{
		pPlayer->SendText("No mutation for that.", LTT_DEFAULT);
	}
...
CLIENT_COMMAND(decent, "[name]", "Gives you great attributes.", SENTINEL_ACCESS)
{
	...
	pPlayer->m_Qualities.SetAttribute(ENDURANCE_ATTRIBUTE, 600);
...
CLIENT_COMMAND(givecredit, "[value]", "Gives you some skill credits for testing.", BASIC_ACCESS)
{
	...
	pPlayer->GiveSkillCredits(1, true);
