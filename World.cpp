...
void CWorld::JuggleEntity(WORD wOld, CWeenieObject* pEntity)
{
	if (!pEntity->HasOwner())
	{
		...
		else
		{
			...
			DWORD generator_id = pEntity->InqIIDQuality(GENERATOR_IID, 0);
			DWORD object_id = pEntity->id;
			...
			if (generator_id)
			{
				...
				if (target)
					target->NotifyGeneratedDeath(object_id);
			}
		}
	}
}
...
CWeenieObject *CWorld::FindObject(DWORD object_id)
{
	if (!object_id)
		return NULL;
...
void CWorld::RemoveEntity(CWeenieObject *pEntity)
{
	...
	if (!pBlock)
	{
		...
		DWORD activation_target_id = pEntity->InqIIDQuality(GENERATOR_IID, 0);
		DWORD object_id = pEntity->id;
...
CWeenieObject *CWorld::FindWithinPVS(CWeenieObject *source, DWORD object_id)
{
	...
	if (CWeenieObject *externalObject = g_pWorld->FindObject(object_id))
	{
		...
		{
			if (externalContainer->_openedBy == source->GetID())
			{
