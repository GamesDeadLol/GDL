	...
	if (_target_id)
	{
		...
		if (target->IsContained())
		{
			...
			if (!_weenie->FindContainedItem(target->GetID()))
			{
				...
				{
					if (externalContainer->_openedBy == _weenie->GetID())
					{
...
void CPickupInventoryUseEvent::OnUseAnimSuccess(DWORD motion)
{
	...
	bool bStored = _weenie->AsContainer()->Container_TryStoreItemNow(_target_id, _target_container_id, _target_slot);
...
void CDropInventoryUseEvent::OnUseAnimSuccess(DWORD motion)
{
	...
	bool bDropped = _weenie->AsContainer()->Container_TryDropItemNow(_target_id);
