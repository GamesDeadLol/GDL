...
{
	for (auto entry : _weenieDefaults)
		delete entry.second;

	_weenieDefaults.clear();
	_weenieDefaultsByName.clear();
	_scrollWeenies.clear();
}
...
{
	auto i = _scrollWeenies.find(spell_id);
	if (i != _scrollWeenies.end())
		return i->second;
...
void CWeenieFactory::MapScrollWCIDs()
{
	...
	for (auto &entry : _weenieDefaults)
	{
		...
		{
			_scrollWeenies[spell_id] = entry.first;
		}
	}
}
...
{
	auto i = _weenieDefaults.find(wcid);
	if (i != _weenieDefaults.end())
		return i->second;
...
CWeenieDefaults *CWeenieFactory::GetWeenieDefaults(const char *name, int index)
{
	...
	std::pair<
		...
		std::multimap<std::string, CWeenieDefaults *>::iterator> range = _weenieDefaultsByName.equal_range(search);
	...
	for (const auto &filePath : filePaths)
	{
		...
		if (fileStream.is_open())
		{
			...
			if (pDefaults->m_Qualities.UnPackJson(jsonData))
			{
				...
				auto existing = _weenieDefaults.find(pDefaults->m_WCID);
				if (existing != _weenieDefaults.end())
				{
					if (refresh)
					{
					}
				...
				if (bShouldInsert)
				{
					...
					pDefaults->m_sourceFile = filePath;
					_weenieDefaults[pDefaults->m_WCID] = pDefaults;
					...
					if (pDefaults->m_Qualities.InqString(NAME_STRING, name))
					{
						...
						// remove if this WCID is already in here
						for (std::multimap<std::string, CWeenieDefaults *>::iterator i = _weenieDefaultsByName.begin(); i != _weenieDefaultsByName.end(); i++)
						{
							if (i->second->m_Qualities.GetID() == pDefaults->m_Qualities.GetID())
							{
								_weenieDefaultsByName.erase(i);
								break;
							}
						}

						_weenieDefaultsByName.insert(std::pair<std::string, CWeenieDefaults *>(name, pDefaults));
					}
				...
				else
				{
					if (bDuplicate)
					{
						LOG_PRIVATE(Data, Warning, "Duplicate WCID %d, ignoring WQF file: \"%s\" due to \"%s\"\n", pDefaults->m_WCID, filePath.c_str(), _weenieDefaults[pDefaults->m_WCID]->m_sourceFile.c_str());
					}
			...
			if (pDefaults->UnPack(&reader))
			{
				...
				*/
				...
				else
				{
					if (bDuplicate)
					{
						LOG(Data, Warning, "Duplicate WCID %d, ignoring due to \"%s\"\n", pDefaults->m_WCID, _weenieDefaults[pDefaults->m_WCID]->m_sourceFile.c_str());
					}
...
void CWeenieFactory::LoadAvatarData()
{
	...
	if (LoadDataFromPhatDataBin(7, &data, &length, 0x591c34e9, 0x2250b020))
	{
		...
		DWORD autoWcid = _firstAvatarWCID = 7000000;
...
CWeenieObject *CWeenieFactory::CreateBaseWeenieByType(int weenieType, unsigned int wcid, const char *weenieName)
{
	...
	case Admin_WeenieType:
		{
			if (wcid == 1)
				weenie = new CPlayerWeenie(NULL, 0, 0);
			else
