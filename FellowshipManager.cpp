...
void Fellowship::UpdateData()
{
	...
	for (auto &entry : _fellowship_table)
	{
		...
		if (player)
		{
			...
			entry.second._share_loot = 0; // TODO
...
FellowshipManager::~FellowshipManager()
{
	for (auto &entry : _fellowshipMap)
		delete entry.second;

	_fellowshipMap.clear();
}

void FellowshipManager::Tick()
{
	if (_nextUpdatesTime <= Timer::cur_time)
	{
		for (auto &fs : _fellowshipMap)
		{
			fs.second->TickUpdate();
		}

		_nextUpdatesTime = Timer::cur_time + 1.0;
	}
}

Fellowship *FellowshipManager::GetFellowship(const std::string &name)
{
	std::unordered_map<std::string, Fellowship *>::iterator entry = _fellowshipMap.find(name);

	if (entry != _fellowshipMap.end())
		return entry->second;
...
int FellowshipManager::Create(const std::string &name, DWORD creator_id, BOOL shareXP)
{
	...
	CWeenieObject *player = g_pWorld->FindPlayer(creator_id);
	if (player)
		player->m_Qualities.SetString(FELLOWSHIP_STRING, name);

	_fellowshipMap[name] = fs;
...
{	
	std::unordered_map<std::string, Fellowship *>::iterator i = _fellowshipMap.find(name);
	if (i == _fellowshipMap.end())
