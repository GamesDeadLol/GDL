...
void CWeenieObject::OnMotionDone(DWORD motion, BOOL success)
{
	...
	if (motion == Motion_Reload)
	{
		...
		case Motion_BowCombat:
			{
				if (CWeenieObject *ammo = GetWieldedCombat(COMBAT_USE_AMMO))
				{
					if (ammo->parent != this)
					{
...
void CWeenieObject::OnDeath(DWORD killer_id)
{
	...
	{
		PackableList<EmoteSet> *emoteSetList = m_Qualities._emote_table->_emote_table.lookup(Death_EmoteCategory);
		...
		if (emoteSetList)
		{
			...
			for (auto &emoteSet : *emoteSetList)
			{
				...
				{
					MakeEmoteManager()->ExecuteEmoteSet(emoteSet, killer_id);
					break;
				}
...
void CWeenieObject::Identify(CWeenieObject *source)
{
	source->SendNetMessage(IdentifyObject(source, this), PRIVATE_MSG, TRUE, TRUE);
}
...
float CWeenieObject::HeadingDiffTo(CWeenieObject *target)
{
...
void CWeenieObject::NotifyGeneratedDeath(DWORD weenie_id)
{
	/*
	if (DWORD generator_iid = InqIIDQuality(GENERATOR_IID, 0))
	{
		if (CWeenieObject *pGenerator = g_pWorld->FindObject(generator_iid))
		{
			pGenerator->OnGeneratedDeath(weenie_id);
		}
	}
	*/

	OnGeneratedDeath(weenie_id);
}

void CWeenieObject::OnGeneratedDeath(DWORD weenie_id)
{
	...
	if (node)
	{
		...
		if (delay >= 0)
		{
			...
			if (_nextRegen < 0.0)
			{
				_nextRegen = Timer::cur_time + InqFloatQuality(REGENERATION_INTERVAL_FLOAT, 0.0, TRUE);
			}
		}
	}
}

void CWeenieObject::NotifyGeneratedPickedUp(DWORD weenie_id)
{
	/*
	if (DWORD generator_iid = InqIIDQuality(GENERATOR_IID, 0))
	{
	...
	*/
	...
	OnGeneratedPickedUp(weenie_id);
	m_Qualities.RemoveInstanceID(GENERATOR_IID);
}

void CWeenieObject::OnGeneratedPickedUp(DWORD weenie_id)
{
	if (!m_Qualities._generator_registry)
		return;
	...
	if (node)
	{
		...
		if (m_Qualities._generator_table)
		{
			for (auto &profile : m_Qualities._generator_table->_profile_list)
			{
				if (profile.slot == oldNode.slot && profile.whenCreate == RegenerationType::PickUp_RegenerationType)
				{
					delay = profile.delay;
					break;
				}
...
void CWeenieObject::GivePerksForKill(CWeenieObject *pKilled)
{
	...
	if (g_pConfig->KillXPMultiplier() != 1.0)
	{
		xpForKill = (int)(xpForKill * g_pConfig->KillXPMultiplier());
	}
...
DWORD CWeenieObject::GiveSkillXP(STypeSkill key, DWORD amount)
{
	Skill skill;
	m_Qualities.InqSkill(key, skill);
	skill._pp += amount;
...
void CWeenieObject::Tick()
{
	...
	if (_nextRegen >= 0 && _nextRegen <= Timer::cur_time)
	{
		...
		CreateGeneratorSpawns(false);

		DWORD numSpawned = (DWORD)(m_Qualities._generator_registry ? (DWORD)m_Qualities._generator_registry->_registry.size() : 0) + (m_Qualities._generator_queue ? (DWORD)m_Qualities._generator_queue->_queue.size() : 0);
		...
		{
			_nextRegen = Timer::cur_time + InqFloatQuality(REGENERATION_INTERVAL_FLOAT, -1.0, TRUE);
		}
	}

	if (_useHeartBeat && _nextHeartBeat <= Timer::cur_time)
	{
		if (!IsDead() && !IsInPortalSpace())
		{
			if ((_last_update_pos + 30.0) <= Timer::cur_time && !IsExecutingEmote())
			{
				if (m_Qualities._emote_table)
				{
...
void CWeenieObject::CheckRegeneration(double rate, STypeAttribute2nd currentAttrib, STypeAttribute2nd maxAttrib)
{
	...
	if (m_Qualities.InqAttribute2nd(currentAttrib, currentVital, FALSE) && m_Qualities.InqAttribute2nd(maxAttrib, maxVital, FALSE))
	{
		...
		{
			currentVital += (DWORD)(ceil(rate) + F_EPSILON);
			if (currentVital > maxVital)
			{
...
{
	CreateGeneratorSpawns(true);
}

void CWeenieObject::CreateGeneratorSpawns(bool init)
{
	if (!m_Qualities._generator_table)
	{
		return;
	...
	/*
	if (m_Qualities.id == W_ERUPTPORTBOSSGEN_CLASS)
	{
		DebugBreak();
	}
	*/
	
	DWORD maxSpawns = init ? InqIntQuality(INIT_GENERATED_OBJECTS_INT, 0) : InqIntQuality(MAX_GENERATED_OBJECTS_INT, 0);
	DWORD numSpawned = 0;
	...
	if (!init)
	{
		if (m_Qualities._generator_registry)
		{
			numSpawned += (DWORD) m_Qualities._generator_registry->_registry.size();
		}

		if (m_Qualities._generator_queue)
		{
			numSpawned += (DWORD)m_Qualities._generator_queue->_queue.size();
		}
	}
	...
	std::list<GeneratorProfile> genList = init ? m_Qualities._generator_table->GetInitialGenerationList() : m_Qualities._generator_table->GetGenerationList();

	// always spawn everything with zero or subzero values
	for (auto &entry : genList)
	{
		...
		if (!IsGeneratorSlotReady(entry.slot))
		{
			continue;
		}

		DWORD maxCreate = init ? entry.initCreate : entry.maxNum;
		for (DWORD i = 0; i < maxCreate; i++)
		{
			if (entry.IsTreasureType())
			{
				if (PackableList<TreasureEntry> *treasureEntry = g_pPortalDataEx->_treasureTableData._wieldedTreasure.lookup(entry.type))
				{
					for (auto &treasure : *treasureEntry)
					{
						// TODO use treasure ptid and shade
						CreateGeneratorSpawn(entry, treasure._wcid, treasure._ptid, treasure._shade);
						numSpawned++;
					}
	...
	// this logic is funky and probably wrong
...
roll_next:
	...
	double dice = Random::RollDice(0.0f, 1.0f);
...
void CWeenieObject::CreateGeneratorSpawn(const GeneratorProfile &profile, DWORD wcid, int ptid, float shade)
{
	if (!wcid)
	{
		return;
...
#if _DEBUG
	// g_pWorld->BroadcastGlobal(ServerText(csprintf("Generating %s (%u)", GetWCIDName(wcid), wcid), LTT_DEFAULT), PRIVATE_MSG, FALSE, FALSE, TRUE);
#endif
	...
	if (!weenie)
	{
		// LOG(Temp, Normal, TEXT("Failed generating spawn %s.\n"), GetWCIDName(profile.type));
		return;
	}

	int amount = 0;
	if (profile.stackSize > 1 && weenie->InqIntQuality(MAX_STACK_SIZE_INT, 1) >= profile.stackSize)
	{
		weenie->SetStackSize(profile.stackSize);
		amount = profile.stackSize;
	}
	...
	case Contain_RegenLocationType:
		if (!AsContainer())
		{
			LOG_PRIVATE(Temp, Warning, "Trying to generate loot in non-container.\n");
			return;
		}

		weenie->SetWeenieContainer(GetID());
		AsContainer()->Container_InsertInventoryItem(0, weenie, 0);
		break;
	...
	case Scatter_RegenLocationType:
		{
			...
			{
				// outdoors
				double genRadius = InqFloatQuality(GENERATOR_RADIUS_FLOAT, 0.0f);
	...
	weenie->m_Qualities.SetInstanceID(GENERATOR_IID, GetID());
	...
	if (weenie->IsCreature() && !weenie->IsStuck() && weenie->m_Position.objcell_id && !weenie->cell)
	{
	...
	m_Qualities._generator_registry->_registry.add(weenie->GetID(), &node);

	// LOG(Temp, Normal, TEXT("Generated spawn %s.\n"), GetWCIDName(profile.type));
}

void CWeenieObject::CreateGeneratorSpawn(const GeneratorProfile &profile)
{
	DWORD wcid = 0;
	int ptid = 0;
	float shade = -1.0f;

	if (profile.IsTreasureType())
	{
		if (PackableList<TreasureEntry> *treasureEntry = g_pPortalDataEx->_treasureTableData._wieldedTreasure.lookup(profile.type))
		{
			for (auto &treasure : *treasureEntry)
			{
				// TODO use treasure ptid and shade
	...
	{
		if (profile.type == W_HUMAN_CLASS || profile.type == W_ADMIN_CLASS || profile.type == W_SENTINEL_CLASS)
			return;
	...
	// treasure 93 green mire grave
	CreateGeneratorSpawn(profile, profile.type, profile.ptid, profile.shade);
}
...
DWORD CWeenieObject::GetCoin()
{
	return m_Qualities.GetInt(COIN_VALUE_INT, 0);
}

void CWeenieObject::SetCoin(DWORD amount)
{
	m_Qualities.SetInt(COIN_VALUE_INT, amount);
}
...
void CWeenieObject::TryToDealDamage(DamageEventData &data)
{
	...
	if (data.damage_form & DF_PHYSICAL)
	{
		...
		if (data.target->m_Qualities._body)
		{
			...
			{
				if (data.hit_quadrant == bp.second._bh)
...
float CWeenieObject::GetEffectiveArmorLevel(DamageEventData &damageData, bool bIgnoreMagicArmor)
{
	...
	int baseArmor = 0;
	if (m_Qualities.InqInt(ARMOR_LEVEL_INT, baseArmor, bIgnoreMagicArmor, FALSE))
	{
		float addedArmor = baseArmor;

		switch (damageData.damage_type)
		{
		case DAMAGE_TYPE::SLASH_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_SLASH_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::PIERCE_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_PIERCE_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::BLUDGEON_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_BLUDGEON_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::FIRE_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_FIRE_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::COLD_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_COLD_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::ACID_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_ACID_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::ELECTRIC_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_ELECTRIC_FLOAT, 1.0f), 2.0f)); break;
		case DAMAGE_TYPE::NETHER_DAMAGE_TYPE: addedArmor *= max(0.0, min(InqFloatQuality(ARMOR_MOD_VS_NETHER_FLOAT, 1.0f), 2.0f)); break;
		}

		if (InqIntQuality(COMBAT_USE_INT, 0, TRUE) == COMBAT_USE::COMBAT_USE_SHIELD)
		{
			...
			addedArmor *= (1.0 - ignoreShieldMod);
		}

		armorLevel += addedArmor;
	}
...
{
	double armorRendingMod = 1.0;

	if (damageData.weapon)
	{
		double damageBonus = 1.0;
		double slayerBonus = 1.0;
		if (damageData.weapon->m_Qualities.InqFloat(SLAYER_DAMAGE_BONUS_FLOAT, slayerBonus, FALSE))
		{
			int slayerCreatureType = damageData.weapon->InqIntQuality(SLAYER_CREATURE_TYPE_INT, 0, TRUE);
			if (slayerCreatureType && slayerCreatureType == InqIntQuality(CREATURE_TYPE_INT, 0, TRUE))
			{
				damageBonus *= slayerBonus;
			}
		}

		if (damageData.weapon->GetImbuedEffects() & ArmorRending_ImbuedEffectType)
		{
			// don't worry about this for now
			// armorRendingMod = (DWORD)max(0.25, min(1.0, 1 - ((damageData.weaponSkill - 100) / 500.0) * 0.75));
		}

		damageData.outputDamage *= damageBonus;
	}
	...
	if (!damageData.ignoreMagicResist)
	{
		switch (damageData.damage_type)
		{
		case SLASH_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_SLASH_FLOAT, 1.0); break;
		case PIERCE_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_PIERCE_FLOAT, 1.0); break;
		case BLUDGEON_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_BLUDGEON_FLOAT, 1.0); break;
		case FIRE_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_FIRE_FLOAT, 1.0); break;
		case COLD_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_COLD_FLOAT, 1.0); break;
		case ACID_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_ACID_FLOAT, 1.0); break;
		case ELECTRIC_DAMAGE_TYPE: damageData.outputDamage *= InqFloatQuality(RESIST_ELECTRIC_FLOAT, 1.0); break;
		}
	}
	...
	if (!damageData.ignoreArmorEntirely && (damageData.damage_form & DF_PHYSICAL))
	{
		...
		double armorLevel = armorRendingMod * GetEffectiveArmorLevel(damageData, damageData.ignoreMagicArmor);

		float damageFactor;

		if (armorLevel >= 0)
			damageFactor = 1.0 / (1.0 + (armorLevel / (200.0 / 3.0)));
		else
			damageFactor = (1.0 + (-armorLevel / (200.0 / 3.0)));

		damageData.outputDamage *= damageFactor;
	}

	if ((damageData.damage_form & DF_MAGIC) && damageData.isProjectile)
	{
		// check for magic absorption
		if (CWeenieObject *shield = GetWieldedCombat(COMBAT_USE::COMBAT_USE_SHIELD))
		{
			if (shield->GetImbuedEffects() & ImbuedEffectType::IgnoreSomeMagicProjectileDamage_ImbuedEffectType)
			{
				...
				if (reduction > 0.0)
				{
					...
					damageData.outputDamage *= 1.0 - reduction;
				}
			}
		}
	}

	if (damageData.outputDamage < 0)
	{
		if (damageData.damage_type & (HEALTH_DAMAGE_TYPE|STAMINA_DAMAGE_TYPE|MANA_DAMAGE_TYPE))
		{
			// these types can heal
			damageData.outputDamageFinal = (int)ceil(damageData.outputDamage - F_EPSILON);
		}
		else
		{
			damageData.outputDamage = 0;
		}
	}
	else
	{
		damageData.outputDamageFinal = (int)floor(damageData.outputDamage + F_EPSILON);
	}
...
bool CWeenieObject::IsContainedWithinViewable(DWORD object_id)
{
	...
	if (CWeenieObject *externalObject = g_pWorld->FindObject(object_id))
	{
		...
		{
			if (externalContainer->_openedBy == GetID())
			{
				return true;
			}
		}
...
int CWeenieObject::SimulateGiveObject(CContainerWeenie *target_container, CWeenieObject *object_weenie)
{
	...
	target_container->ReceiveInventoryItem(this, object_weenie, 0);
...
double CWeenieObject::GetDefenseMod()
{
	...
	{
		mod *= wielder->GetDefenseMod();
	}

	return mod;
}

double CWeenieObject::GetDefenseModUsingWielded()
{
	return GetDefenseMod();
}
...
double CWeenieObject::GetCriticalMultiplier()
{
	double mod = InqFloatQuality(CRITICAL_MULTIPLIER_FLOAT, 2.0, FALSE);
...
double CWeenieObject::GetMeleeCripplingBlowMultiplier(DWORD meleeSkill)
{
	double increase = (meleeSkill - 40) / 60.0;
	increase *= max(1.0, increase);

	return increase;
}

double CWeenieObject::GetMissileCripplingBlowMultiplier(DWORD missileSkill)
{
	double increase = (missileSkill) / 60.0;
	increase *= max(1.0, increase);

	return increase;
}

double CWeenieObject::GetCriticalFrequency()
{
	double mod = InqFloatQuality(CRITICAL_FREQUENCY_FLOAT, 0.1, FALSE);
...
double CWeenieObject::GetMeleeCriticalStrikeFrequency(DWORD meleeSkill)
{
	double chance = (meleeSkill - 100) / 600.0;
...
double CWeenieObject::GetMissileCriticalStrikeFrequency(DWORD missileSkill)
{
	double chance = (missileSkill - 60) / 600.0;
...
double CWeenieObject::GetCasterCriticalStrikeFrequency(DWORD casterSkill)
{
	double chance = (casterSkill - 60) / 600.0;
...
DWORD CWeenieObject::GetImbuedEffects()
{
...
bool CWeenieObject::TryMeleeEvade(DWORD attackSkill)
{
	...
	defenseSkill = (int)(defenseSkill * GetDefenseModUsingWielded());
	...
	defenseSkill = (int)(defenseSkill * defenseMod);

	return ::TryMeleeEvade(attackSkill, defenseSkill);
}
...
bool CWeenieObject::TryMissileEvade(DWORD attackSkill)
{
	...
	return ::TryMissileEvade(attackSkill, defenseSkill);
}

void CWeenieObject::OnIdentifyAttempted(CWeenieObject *other)
{
}
...
void CWeenieObject::ResistedSpell(CWeenieObject *attacker)
{
}

void CWeenieObject::EvadedAttack(CWeenieObject *attacker)
{
}
...
void CWeenieObject::DecrementStackOrStructureNum(bool bDestroyOnZero)
{
	int dummy;
	if (m_Qualities.InqInt(STRUCTURE_INT, dummy, TRUE))
	{
		DecrementStructureNum(bDestroyOnZero);
	}
	else
	{
		DecrementStackNum(bDestroyOnZero);
	}
}

void CWeenieObject::DecrementStackNum(bool bDestroyOnZero)
{
	...
	{
		// out of uses, destroy it
		if (CContainerWeenie *player = GetWorldTopLevelContainer())
		{
			ReleaseFromAnyWeenieParent();

			BinaryWriter removeInventoryObjectMessage;
			removeInventoryObjectMessage.Write<DWORD>(0x24);
			removeInventoryObjectMessage.Write<DWORD>(GetID());
...
void CWeenieObject::DecrementStructureNum(bool bDestroyOnZero)
{
...
BOOL CWeenieObject::InqSkill(STypeSkill key, DWORD &value, BOOL raw)
{
	...
	if (!AsPlayer() && bResult)
	{
		...
		switch (key)
		{
		case MELEE_DEFENSE_SKILL:
		case MISSILE_DEFENSE_SKILL:
		case MAGIC_DEFENSE_SKILL:
			{
				// temporary while item buffs don't work perhaps
				DWORD baseValue = 0;
				if (m_Qualities.InqSkillBaseLevel(key, baseValue, raw))
				{
					value -= baseValue;
				}

				// value = (DWORD)value * 0.8;
				break;
			}
		}
		*/
...
{
	return InqIntQuality(BONDED_INT, 0) == 1 ? true : false;
}
...
{
	return InqIntQuality(BONDED_INT, 0) == -2 ? true : false;
}
