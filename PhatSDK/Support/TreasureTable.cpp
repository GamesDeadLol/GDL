...
DEFINE_UNPACK(TreasureEntry)
{
	...
	m_b28 = pReader->Read<BOOL>(); // bool
	m_b2C = pReader->Read<BOOL>(); // bool
	m_b30 = pReader->Read<BOOL>(); // bool
...
DEFINE_UNPACK(TreasureTable)
{
	...
	_wieldedTreasure.UnPack(pReader);	
	_treasure2.UnPack(pReader);	
...
{
	auto entry = _treasure2.lookup(treasure_id);
...
{
	float dice = g_pPhatSDK->GetRandomFloat(0.0, 1.0);
...
int TreasureTable::RollPaletteTemplateIDFromMaterialAndColorCode(MaterialType baseMaterial, int colorCode, int tier)
{
	auto materialColorMap = _materialColorKeyMap.lookup(baseMaterial);

	if (!materialColorMap || tier < 1 || tier > 6)
	{
