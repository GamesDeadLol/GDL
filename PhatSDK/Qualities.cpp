...
BOOL Enchantment::Enchant(EnchantedQualityDetails *value)
{
	...
	if (!(_smod.type & Additive_EnchantmentType))
	{
		if (_smod.type & Multiplicative_EnchantmentType)
		{
			if (_smod.val <= 1.0)
			{
				value->rValueDecreasingMultiplier = _smod.val * value->rValueDecreasingMultiplier;
				bEnchanted = TRUE;
			}
			else
			{
				value->rValueIncreasingMultiplier = _smod.val * value->rValueIncreasingMultiplier;
				bEnchanted = TRUE;
			}
		}
	}
	...
	{
		value->rValueDecreasingAdditive = _smod.val + value->rValueDecreasingAdditive;
		bEnchanted = TRUE;
	}
	else
	{
		value->rValueIncreasingAdditive = _smod.val + value->rValueIncreasingAdditive;
		bEnchanted = TRUE;
	}
